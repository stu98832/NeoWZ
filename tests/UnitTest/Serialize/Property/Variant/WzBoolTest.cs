...
{
    [TestClass]
    public class WzBoolTest
    {
        private WzVariant Variant = new WzBool("variant", true);
        private WzVariant Variant2 = new WzBool("variant2", false);
        ...
        [TestMethod]
        public void ToTextTest() => Assert.AreEqual("true", Variant.ToText("text"));

        [TestMethod]
        public void ToComTest() => Assert.AreEqual(null, Variant.ToCom<IComSerializable>());

        [TestMethod]
        public void EqualsTest() => Assert.AreEqual(new WzBool("variant", true), Variant);

        [TestMethod]
        public void EqualsFailedTest() => Assert.AreNotEqual(new WzBool("variant"), Variant);
    }
}
